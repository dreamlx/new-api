...
// Payment-domain money helpers. Kept in the service package (not common/)
// because these conversions encode payment-specific rules (cents=fen,
// banker's-style rounding, Alipay's "%.2f" wire format). Putting them in
// common/ would invite reuse for non-payment money math where the
// rounding policy is incorrect.
