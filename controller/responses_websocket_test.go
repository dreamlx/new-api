...
func TestResponsesWSRequestRunnerRejectsRevokedCredentials(t *testing.T) {
	for _, tc := range []struct {
		...
		// Fork divergence (lh-main): a disabled (revoked) token must yield 403
		// api_key_revoked so downstream consumers can distinguish revocation
		// from an invalid key (LH ADR-024 cutover contract, commit 6cf3b0531).
		// Upstream expects 401 here.
		{name: "disabled token", status: http.StatusForbidden, update: func(t *testing.T, _ *model.User, token *model.Token) {
