...
// RechargePayPal completes a pending PayPal topup order. PayPal stores its USD
// amount in TopUp.Money (already group-ratio-adjusted), mirroring the Stripe flow.
func RechargePayPal(referenceId string, callerIp string) (err error) {
	...
	RecordTopupLog(topUp.UserId, fmt.Sprintf("使用PayPal充值成功，充值金额: %v，支付金额：%.2f USD", logger.FormatQuota(int(quota)), topUp.Money), callerIp, topUp.PaymentMethod, "paypal")
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.PaymentMethod == PaymentMethodStripe || topUp.PaymentMethod == "paypal" {
