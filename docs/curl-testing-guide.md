...
本系统中的API Token采用**共享用户余额机制**，所有属于同一用户的Token都共享该用户的统一余额池。

#### 核心机制说明

1. **统一余额池**: 每个用户都有一个主账户余额（`user.quota`），这是该用户所有Token共享的余额池。
2. **Token不再独立计费**: Token表中的`remain_quota`字段仅作为历史记录保留，实际额度检查和扣费都基于用户主账户余额。
3. **实时余额检查**: 当使用任何Token进行API调用时，系统直接检查用户主账户余额，而不是Token的独立余额。

#### 工作流程

- **Token验证**: 检查用户主账户余额是否充足
- **额度扣减**: 直接从用户主账户余额中扣减，不操作Token余额
- **余额共享**: 同一用户的所有Token都能使用相同的余额池
...
sequenceDiagram
    ...
    Note over Client,DB: Token共享用户余额流程
    Client->>API: 1. 创建/同步用户
    API->>DB: 用户余额初始为0
    Client->>API: 2. 创建Token
    API->>DB: Token创建(不依赖独立余额)
    Client->>API: 3. 用户充值
    API->>DB: 更新用户余额=500,000
    Client->>API: 4. 使用Token调用API
    API->>DB: 直接检查用户余额=500,000
    API->>DB: 从用户余额中扣减费用
    API->>Client: API调用成功
    Note over Client,DB: 所有Token共享同一用户余额池
```

#### 机制优势

- **🔄 实时共享**: 所有Token立即共享用户的最新余额
- **🚫 避免负数**: 消除了双重扣费和竞态条件问题
- **⚡ 性能优化**: 减少了额度同步的复杂逻辑
- **🎯 逻辑简化**: Token不再需要维护独立的余额状态
...
2. **为用户充值** → 确保账户有足够余额
3. **创建Token** → 获得访问凭证
4. **使用Token调用API** → 直接使用共享余额池

#### ⚠️ 重要提示

- **余额池共享**: 同一用户的所有Token都使用相同的余额池
- **即时生效**: 充值后所有Token立即可用，无需等待或同步
